...
export default function AdminDashboardPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'CONFIRMED':
        return <span className="bg-blue-100 text-blue-800 font-bold px-2 py-0.5 rounded text-[10px]">CONFIRMED</span>;
      case 'PACKING':
      case 'PACKED':
        return <span className="bg-indigo-100 text-indigo-800 font-bold px-2 py-0.5 rounded text-[10px]">PACKING</span>;
      case 'DISPATCHED':
        return <span className="bg-purple-100 text-purple-800 font-bold px-2 py-0.5 rounded text-[10px]">DISPATCHED</span>;
      case 'DELIVERED':
        return <span className="bg-emerald-100 text-emerald-800 font-bold px-2 py-0.5 rounded text-[10px]">DELIVERED</span>;
      case 'CANCELLED':
        return <span className="bg-red-100 text-red-800 font-bold px-2 py-0.5 rounded text-[10px]">CANCELLED</span>;
      default:
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/90 shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={loadData}
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-xs transition-colors cursor-pointer font-heading"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/90 shadow-2xs space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider font-heading">
              Total Revenue
            </span>
          ...
          <p className="text-lg sm:text-2xl font-black text-slate-950 tracking-tight font-heading">
        ...
        <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/90 shadow-2xs space-y-2">
          ...
          <p className="text-lg sm:text-2xl font-black text-emerald-700 tracking-tight font-heading">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-8 bg-white p-4 sm:p-6 rounded-3xl border border-slate-200/90 shadow-2xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h2 className="font-black text-sm sm:text-base text-slate-950 flex items-center gap-2 font-heading">
              <PackageCheck className="w-4 h-4 text-amber-600" />
              <span>Recent Customer Orders</span>
            </h2>
            <Link
              href="/admin/orders"
              className="text-xs font-bold text-amber-600 hover:text-amber-700 flex items-center gap-1 font-heading"
            >
              ...
              recentOrders.map((order) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="text-slate-500 font-medium block truncate mt-0.5">
                      {order.customer_name} • {order.city}, {order.state}
                    </span>
                  </div>

                  <div className="text-right shrink-0">
                    <span className="font-black text-slate-950 block font-heading">
          ...
          <div className="space-y-2">
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-bold text-slate-900">Manage Orders</span>
              </div>
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-bold text-slate-900">Manage Product Catalog</span>
              </div>
              <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:translate-x-0.5 transition-transform" />
            </Link>
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-bold text-slate-900">Manage Categories</span>
              </div>
              <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:translate-x-0.5 transition-transform" />
            </Link>
          </div>
